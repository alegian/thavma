...
object NodeAbsorbRenderer {
  fun renderLevelAfterEntities(event: RenderLevelStageEvent) {
    ...
    for (player in players) {
      ...
      // todo: get target node blockpos via data comp
      val targetPos = Vec3.ZERO

      event.poseStack.use {
        setUpWandPose(player, playerRenderer, partialTick)
        render(event.poseStack, ClientHelper.bufferSource(), partialTick, player.level().gameTime, targetPos)
      }
    }
  }

  fun render(handPose: PoseStack, bufferSource: MultiBufferSource, partialTick: Float, gameTime: Long, targetPos: Vec3) {
    handPose.use {
      val wandTipPos = transformOrigin().toVec3()
      val length = trajectoryLength(targetPos, wandTipPos)
      val colorWithAlpha = 0x44000000 or (T7Colors.PURPLE and 0xffffff)
      renderFlyingAspects(targetPos, wandTipPos, 0.2, length - 1, length, this, bufferSource, gameTime + partialTick, colorWithAlpha, 0.06)
    }
  }
}