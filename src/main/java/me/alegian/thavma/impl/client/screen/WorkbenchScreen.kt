...
open class WorkbenchScreen(val menu: WorkbenchMenu, pPlayerInventory: Inventory, pTitle: Component) : T7ContainerScreen<WorkbenchMenu>(menu, pPlayerInventory, pTitle, WORKBENCH_BG) {
  override fun layout() {
    Row(padding = Padding(BORDER), gap = BORDER) {
      addRenderableOnly(debugRect(0xFF0000FF.toInt()))
      Column(padding = Padding(BORDER), gap = BORDER) {
        addRenderableOnly(debugRect(0xFFFFFFFF.toInt()))
        Box(size = Size(50, 50)) {
          addRenderableOnly(debugRect(0xFFFF00FF.toInt()))
        }
        Box(size = Size(50, 50)) {
          addRenderableOnly(debugRect(0xFFFF00FF.toInt()))
        }
